...
from transformers import (
    ...
    EnsembleForOffensiveClassification,
)
from transformers import glue_compute_metrics as compute_metrics
from transformers import ensemble_convert_examples_to_features as convert_examples_to_features
...
def train(args, train_dataset, model, bert_tokenizer, roberta_tokenizer):
    """ Train the model """
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (
                        ...
                        results = evaluate(args, model, bert_tokenizer, roberta_tokenizer)
...
def evaluate(args, model, bert_tokenizer, roberta_tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset = load_and_cache_examples(args, eval_task, bert_tokenizer, roberta_tokenizer, evaluate=True)
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            if preds is None:
                ...
                out_label_ids = inputs["classification_labels"].detach().cpu().numpy()
            else:
...
def load_and_cache_examples(args, task, bert_tokenizer, roberta_tokenizer, evaluate=False):
...
def main():
    ...
    model = ensemble_model.from_pretrained(
    ...
    model.ftcbert.to("cpu")
    model.ptftcbert.to("cpu")
    model.ptftrbert.to("cpu")
    model.ftcroberta.to("cpu")
    model.ptftcroberta.to("cpu")
    model.ptftrroberta.to("cpu")
    logger.info("Training/evaluation parameters %s", args)

    # Training
    if args.do_train:
        train_dataset = load_and_cache_examples(args, args.task_name, bert_tokenizer,roberta_tokenizer, evaluate=False)
        global_step, tr_loss = train(args, train_dataset, model, bert_tokenizer, roberta_tokenizer)
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        roberta_tokenizer.save_pretrained(args.output_dir)

        # Good practice: save your training arguments together with the trained model
        torch.save(args, os.path.join(args.output_dir, "training_args.bin"))

        # Load a trained model and vocabulary that you have fine-tuned
        model = ensemble_model.from_pretrained(args.output_dir)
        bert_tokenizer = bert_tokenizer_class.from_pretrained(args.output_dir)
        roberta_tokenizer = roberta_tokenizer_class.from_pretrained(args.output_dir)
        model.to(args.device)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        roberta_tokenizer = roberta_tokenizer_class.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case)
        ...
        for checkpoint in checkpoints:
            ...
            model = ensemble_model.from_pretrained(checkpoint)
            model.to(args.device)
            result = evaluate(args, model, bert_tokenizer,roberta_tokenizer, prefix=prefix)
